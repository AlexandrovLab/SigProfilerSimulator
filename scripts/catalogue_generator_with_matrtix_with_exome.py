...
def catalogue_generator_single (vcf_path, vcf_files, chrom_path, chromosome_TSB_path, test, output_matrix, context, exome, genome, ncbi_chrom):
            ...
            for lines in f:
                ...
                # Saves all relevant data from the file
                    ...
                    if flag:
                        ...
                        # Opens the TSB information if necessary
    ...
    matrix_generator (context, output_matrix, test, samples, bias_sort, mutation_dict, types, exome)
...
def matrix_generator (context, output_matrix, test, samples, bias_sort, mutation_dict, types, exome):
    '''
...
def main():
    ...
    parser.add_argument("test",help="Provide a unique name for your samples. (ex: BRCA)")
    parser.add_argument("genome",help="Provide a reference genome. (ex: GRCh37, GRCh38, mm10)")
    parser.add_argument("context",help="Whole genome context by default")
    ...
    if context != 'DINUC' and context != 'INDEL':
        catalogue_generator_single (vcf_path, vcf_files, chrom_path, chromosome_TSB_path, test, output_matrix, context, args.exome, args.genome, ncbi_chrom)

    elif context == 'DINUC':
        catalogue_generator_DINUC_single (vcf_path, vcf_files, chrom_path, chromosome_TSB_path, test, output_matrix, args.exome, args.genome, ncbi_chrom)

    elif context == 'INDEL':
        catalogue_generator_INDEL_single (vcf_path, vcf_files, chrom_path, test, output_matrix, args.exome, args.genome, ncbi_chrom)
