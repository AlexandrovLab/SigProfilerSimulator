...
def probability_mask(chrom, mask):
    ...
    if chrom not in maskMatrix.index:
        ...
        maskRanges = [
            [x, y]
            for x, y in zip(list(maskMatrix["Start"]), list(maskMatrix["Lengths"]))
        ]
        maskProbs = list(maskMatrix["Probability"])
        if abs(sum(maskProbs) - 1) > 0.01:
            print(
                "Mutation rate mask distribution is incomplete. Please ensure that the distribution probabilites sums to 1 across each chromosome."
            )
            sys.exit()
        return (maskRanges, maskProbs)
...
def combine_simulation_files(
    iterations, output_path, chromosomes, samples=[], bed=False, exome=False, vcf=False
):
    """
    ...
    if vcf:
        for sample in samples:
            for i in iterations:
                with open(
                    output_path + sample + "/" + sample + "_" + str(i) + ".vcf", "w"
                ) as f:
                    print(
                        "\t".join(
                            [
                ...
                with open(
                    output_path + sample + "/" + sample + "_" + str(i) + ".vcf", "ab"
                ) as f:
        ...
        for i in iterations:
            ...
            with open(output_path + str(i) + ".maf", "ab") as f:
                for chrom in chromosomes:
                    try:
                        with open(
                            output_path + str(i) + "_" + chrom + extension + ".maf",
                            "rb",
                        ) as fd:
                            shutil.copyfileobj(fd, f)
                        os.remove(
...
def mutation_preparation_chromosomes(
                ...
                for lines in f:
                    ...
                    for sample in current_samples:
                        ...
                        if sample not in mutation_tracker[context]:
                            ...
                            mutation_tracker[context][sample][nuc][chrom] = int(
                                mutCount
                            )
                        else:
...
def mutation_preparation_region(
...
def mut_tracker(
    ...
    for context in nucleotide_context_files:
                ...
                for nuc in samples[context][random_sample]:
                    ...
                    for chroms in chromosomes:
                        try:
                            ...
                            mutation_count = (
                                int(samples[context][sample][nuc])
                                * nuc_probs[base_nuc][chrom_index]
                            )
        ...
        # of each chromosome in relation to the overall size of the genome.
            ...
            if bed:
                with open(
                    chromosome_string_path + "BED_" + genome + "_proportions.txt", "rb"
                ) as probs:
                    chromosomeProbs = pickle.load(probs)
            else:
                with open(
                    chromosome_string_path + genome + "_proportions.txt", "rb"
                ) as probs:
                    chromosomeProbs = pickle.load(probs)
...
def simulator(
    ...
    for chrom in chromosomes:
        ...
        if (
            ...
            or "384" in contexts
            or "6144" in contexts
            or "DBS186" in contexts
            or "24" in contexts
            or "ID415" in contexts
            or "288" in contexts
            or "4608" in contexts
        ):
                ...
                for lines in f:
                    ...
                    if range_bias == 0:
                        ...
                        if chrom_bias_lengths["N"] == []:
                            ...
                            chrom_bias_lengths["N"].append(
                                chrom_bias_lengths["N"][-1] + (end - start)
                            )
                    elif range_bias == 1:
                        ...
                        if chrom_bias_lengths["TU"] == []:
                            ...
                            chrom_bias_lengths["TU"].append(
                                chrom_bias_lengths["TU"][-1] + (end - start)
                            )
                    elif range_bias == 2:
                        chrom_bias["TU"].append([start, end])
                        if chrom_bias_lengths["TU"] == []:
                        ...
                        if "288" in contexts or "4608" in contexts:
                            ...
                            if chrom_bias_lengths["B"] == []:
                                ...
                                chrom_bias_lengths["B"].append(
                                    chrom_bias_lengths["B"][-1] + (end - start)
                                )
        ...
        for sample in sample_names:
            ...
            while simulations > 0:
                ...
                if vcf:
                    outputFile = "".join(
                        [sample_path, sample, "_", str(simulations), "_", chrom, ".vcf"]
                    )
                else:
                    outputFile = "".join(
                        [sample_path, str(simulations), "_", chrom, ".maf"]
                    )
                ...
                with open(outputFile, "a", 10000000) as out_vcf:
                    for context in contexts:
                        ...
                        if context == "6":
                            ...
                            context == "3072" or context == "6144" or context == "4608"
                        ):
                            sim = 7
                            mut_save = 5
                            mut_start = 2
                        ...
                        random_sample = random.sample(
                            list(mutation_tracker[context]), 1
                        )[0]

                        for nuc in mutation_tracker[context][random_sample]:
                            mutationsCount[nuc] = mutation_tracker[context][sample][
                                nuc
                            ][chrom]
                        if (
                            sample in left_over_mutations
                            and simulations in left_over_mutations[sample]
                        ):

                            if any(left_over_mutations[sample][simulations]):
                                for nuc in left_over_mutations[sample][simulations][
                                    context
                                ]:
                                    try:
                                        mutationsCount[nuc] += left_over_mutations[
                                            sample
                                        ][simulations][context][nuc]
                                    except:
                                        mutationsCount[nuc] = left_over_mutations[
                                            sample
                                        ][simulations][context][nuc]
                        ...
                        if noisePoisson or noiseUniform != 0:
                            mutationsCount = noise(
                                mutationsCount, noisePoisson, noiseUniform
                            )
                        ...
                        if sim == 6:
                            ...
                            for indels in mutationsCount:
                                ...
                                if indel[3] == "0" and indel[1] == "Ins":
                                    if (
                                        indel[0] + indel[3] + indel[2] + save_key
                                    ) not in indel_types_O:
                                        indel_types_O[
                                            (indel[0] + indel[3] + indel[2] + save_key)
                                        ] = [indel[1]]
                            ...
                            while any(mutationsCount) == True:
                                while (any(indel_types)) == True:
                                    ...
                                    break_flag = False
                                    if maskRanges:
                                        weighted_range = maskRanges[
                                            np.random.choice(
                                                len(maskRanges), p=maskProbs
                                            )
                                        ]
                                        random_number = (
                                            fastrand.pcg32bounded(weighted_range[1])
                                            + weighted_range[0]
                                        )
                                    else:
                                        random_number = fastrand.pcg32bounded(
                                            location_range
                                        )
                                    if bed:
                                    ...
                                    if (
                                        random_number in recorded_positions
                                        and not overlap
                                    ):
                                        continue
                                    u += 1
                                    ...
                                    for i in range(max(indel_lengths), 0, -1):
                                        ...
                                        try:
                                            for r in range(
                                                random_number, i + random_number, 1
                                            ):
                                                if r > len(sequence):
                                                    break
                                        ...
                                        if "N" not in inDel:
                                            ...
                                            mainType = str(i) + str(
                                                repeat_count + repeat_count_ins
                                            )
                                            mainType_ins = str(i) + str(
                                                repeat_count + 1 + repeat_count_ins
                                            )
                                            ...
                                            if i != 1:
                                                ...
                                                if (
                                                    tsb_ref[sequence[random_number]][1]
                                                    == "A"
                                                    or tsb_ref[sequence[random_number]][
                                                        1
                                                    ]
                                                    == "T"
                                                ):
                                            ...
                                            if mainType in indel_types:
                                                if indel_types[mainType][0] == "Del":
                                                    ...
                                                    u = 0
                                                    if (
                                                        mutationsCount[complete_indel]
                                                        == 0
                                                    ):
                                                        del mutationsCount[
                                                            complete_indel
                                                        ]
                                                        indel_lengths.remove(
                                                            int(mainType[0])
                                                        )
                                                        repeat_lengths.remove(
                                                            repeat_count
                                                            + repeat_count_ins
                                                        )
                                                        if (
                                                            len(indel_types[mainType])
                                                            > 1
                                                        ):
                                            ...
                                            # Simulates microhomology deletions
                                                ...
                                                # homology equals 0.
                                                if (
                                                    mainType1 in indel_types
                                                    and homology_size2 == 0
                                                ):
                                                    if not overlap:
                                                        for p in range(
                                                            random_number,
                                                    ...
                                                    if bool(
                                                        re.match("^[CT]*$", seq_final)
                                                    ) or bool(
                                                        re.match("^[GA]*$", seq_final)
                                                    ):
                                                        if bool(
                                                            re.match(
                                                                "^[GA]*$", seq_final
                                                ...
                                                # homology equals 0.
                                                    ...
                                                    mainType2 in indel_types
                                                    and homology_size1 == 0
                                                ):
                                                    seq = ""
                                                    try:
                                                        for r in range(
                                ...
                                for indels_M in indel_types_M:
                                    ...
                                    if not bool(
                                        re.match("^[CT]*$", inDel)
                                    ) and not bool(re.match("^[GA]*$", inDel)):
                                        bias = "Q"
                                        strand = "Q"
                                    else:
                                        if bool(re.match("^[GA]*$", inDel)):
                                    ...
                                    if "N" not in potential_sequence:
                                        ...
                                        try:
                                            ...
                                            for r in range(
                                                random_number,
                                                random_number + int(indels_M[1]) + 1,
                                                1,
                                            ):
                                                if r > len(sequence):
                                                    break
                                        ...
                                        if (
                                            seq
                                            != potential_sequence[-int(indels_M[1]) :]
                                            and seq2
                                            != potential_sequence[: int(indels_M[1])]
                                            and seq3
                                            != potential_sequence[
                                ...
                                while any(indel_types_O) == True:
                                    ...
                                    strand = "+"
                                    if (
                                        tsb_ref[sequence[random_number]][1] == "T"
                                        or tsb_ref[sequence[random_number]][1] == "A"
                                    ):
                                        subType = "T"
                                    elif (
                                        tsb_ref[sequence[random_number]][1] == "G"
                                        or tsb_ref[sequence[random_number]][1] == "C"
                                    ):
                                        subType = "C"
                                    else:
                                        break
                        ...
                        # Simulations for INDEL with TSB
                            ...
                            for indels in mutationsCount:
                                ...
                                if bias == "T" or bias == "U":
                                    ...
                                    refbias = bias
                                indel = indel[1:]
                                mutationsCountTSB[refbias][indels] = mutationsCount[
                                    indels
                                ]
                                if (
                                    int(indel[0]) == 5
                                ...
                                if indel[3] == "0" and indel[1] == "Ins":
                                    if (
                                        indel[0] + indel[3] + indel[2] + bias
                                    ) not in indel_types_O[refbias]:
                                        indel_types_O[refbias][
                                            (indel[0] + indel[3] + indel[2] + bias)
                                        ] = [indel[1]]
                            ...
                            for tsb_type in tsb:
                                ...
                                while any(mutationsCountTSB[tsb_type]) == True:
                                    while (any(indel_types[tsb_type])) == True:
                                        ...
                                        if not bed:
                                            if tsb_type != "Q":
                                                location_range = chrom_bias_lengths[
                                                    tsb_type
                                                ][-1]
                                                if maskRanges:
                                                    weighted_range = maskRanges[
                                                    ...
                                                    specific_range = bisect.bisect_left(
                                                        chrom_bias_lengths[tsb_type],
                                                        random_range,
                                                    )
                                                    random_number = (
                                                        chrom_bias_lengths[tsb_type][
                                            ...
                                            if tsb_type != "Q":
                                                location_range = len(chrom_range)
                                                random_range = fastrand.pcg32bounded(
                                                    location_range
                                                )
                                                random_number = chrom_range[
                                                    random_range
                                                ]
                                            else:
                                                random_number = fastrand.pcg32bounded(
                                        ...
                                        # Pulls out the largest desired INDEL
                                        for i in range(
                                            max(indel_lengths[tsb_type]), 0, -1
                                        ):
                                            strand = "+"
                                            micro = False
                                            bias = tsb_ref[sequence[random_number]][0]
                                            ...
                                            if bool(re.match("^[GA]*$", inDel)) or bool(
                                                re.match("^[CT]*$", inDel)
                                            ):
                                                if bool(re.match("^[GA]*$", inDel)):
                                                    strand = "-"
                                                ...
                                                if mainType in indel_types[tsb_type]:
                                                    if (
                                                        ...
                                                        if (
                                                            ...
                                                            del mutationsCountTSB[
                                                                tsb_type
                                                            ][complete_indel]
                                                            indel_lengths[
                                                                tsb_type
                                                            ].remove(int(mainType[0]))
                                                            repeat_lengths[
                                                                tsb_type
                                                            ].remove(
                                                                repeat_count
                                    ...
                                    while any(indel_types_O[tsb_type]) == True:
                                        ...
                                        if (
                                            tsb_ref[sequence[random_number]][1] == "T"
                                            or tsb_ref[sequence[random_number]][1]
                                            == "A"
                                        ):
                                            subType = "T"
                                            if (
                                        ...
                                        for indels_O in indel_types_O[tsb_type]:
                                            ...
                                            if int(indels_O[0]) == 1:
                                                ...
                                                if (
                                                    potential_sequence != indels_O[2]
                                                    and revcompl(potential_sequence)
                                                    != indels_O[2]
                                                ):
                                                    continue
                                            # Randomly chooses bases until the insertion length equals the desired
                                            # INDEL length
                                                ...
                                                for m in range(0, int(indels_O[0]), 1):
                                                    potential_sequence += random_base(
                                                        0, 3
                                                    )
                                            ...
                                            if bias != "N":
                                                if not bool(
                                                    re.match(
                                                        "^[CT]*$", potential_sequence
                                                    )
                                                ) and not bool(
                                                    re.match(
                                                        "^[GA]*$", potential_sequence
                                                    )
                                                ):
                                                    bias = "Q"
                                                else:
                                            ...
                                            if "N" not in seq:
                                                if (
                                                    ...
                                                    if (
                                                        ...
                                                        del mutationsCountTSB[tsb_type][
                                                            complete_indel
                                                        ]
                                                        indel_lengths[tsb_type].remove(
                                                            int(indels_O[0])
                                                        )
                                                        repeat_lengths[tsb_type].remove(
                                                            int(indels_O[1])
                                                        )
                                                        del indel_types_O[tsb_type][
                                                            indels_O
                                                        ]
                                                        # break
                        ...
                        # Simulates 96, 1536, and DINUCs
                            ...
                            while any(mutationsCount) == True:
                                ...
                                if maskRanges:
                                    weighted_range = maskRanges[
                                        np.random.choice(len(maskRanges), p=maskProbs)
                                    ]
                                    random_number = (
                                        fastrand.pcg32bounded(weighted_range[1])
                                        + weighted_range[0]
                                ...
                                if mutNuc in base_keys:
                                    ...
                                    if (
                                        nuc_keys[nucIndex] in mutationsCount
                                        and mutationsCount[nuc_keys[nucIndex]] != 0
                                    ):
                                        ...
                                        if sim != 5 and sim != 4 and sim != 7:
                                            ...
                                            context_up = "DBS"
                                            bases = nuc_keys[nucIndex][
                                                mut_save + 3 : mut_save + 5
                                            ]
                                            if vcf:
                                                print(
                                                    "".join(
                                ...
                                if updating and bases != None:
                                    bias = tsb_ref[sequence[random_number]][0]
                                    new_bases = "".join(
                                        [str(tsb_ref_rev[bias][base]) for base in bases]
                                    )
                        ...
                        # Simulates TSB [24, 288, 384, 6144]
                            ...
                            for nuc in nuc_keys:
                                ...
                                if sim == 10:
                                    ...
                                    base_keys[nuc[0]].append(
                                        nuc[0] + nuc[2] + nuc[4] + nuc[8]
                                    )
                                elif sim == 7:
                                    base_keys[nuc[0]].append(
                                        nuc[0] + nuc[2:4] + nuc[5] + nuc[9:]
                                    )
                                elif sim == 8:
                                ...
                                while any(mutationsCountTSB[tsb_type]) == True:
                                    ...
                                    if l > 1000000:
                                        ...
                                        if sample not in left_over_mutations:
                                            ...
                                            if (
                                                simulations
                                                not in left_over_mutations[sample]
                                            ):
                                                left_over_mutations[sample][
                                                    simulations
                                                ] = {}
                                    ...
                                    if (
                                        "288" in contexts or "4608" in contexts
                                    ) and nuc_bias == "B":
                                    ...
                                    revCompMutNuc = revbias(nuc_bias) + revcompl(
                                        mutNuc[1:]
                                    )
                                    if tsb_type == "Q":
                                        if (
                                            mutNuc in dinuc_non_tsb
                                            or revCompMutNuc in dinuc_non_tsb
                                        ):
                                            l -= 1
                                            continue
                                    ...
                                    if mutNuc in base_keys[nuc_bias]:
                                        ...
                                        if (
                                            nuc_keys_tsb[nuc_bias][nucIndex]
                                            in mutationsCountTSB[tsb_type]
                                            and mutationsCountTSB[tsb_type][
                                                nuc_keys_tsb[nuc_bias][nucIndex]
                                            ]
                                            != 0
                                        ):
                                    ...
                                    # write it to the output file as the reverse complement.
                                        ...
                                        nucIndex = base_keys[rev_tsb_type].index(
                                            revCompMutNuc
                                        )
                                        if (
                                            nuc_keys_tsb[rev_tsb_type][nucIndex]
                                            in mutationsCountTSB[tsb_type]
                                            and mutationsCountTSB[tsb_type][
                                                nuc_keys_tsb[rev_tsb_type][nucIndex]
                                            ]
                                    ...
                                    if updating and bases != None:
                                        bias = tsb_ref[sequence[random_number]][0]
                                        new_bases = "".join(
                                            [tsb_ref_rev[bias][base] for base in bases]
                                        )
                                        sequence = update_chromosome(
                                            sequence,
                                            random_number,
